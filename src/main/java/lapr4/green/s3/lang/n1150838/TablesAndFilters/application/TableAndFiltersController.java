...
//    }
    public List<Row> verifyRows(Table d, String s) throws FormulaCompilationException, IllegalValueTypeException {
        List<Row> invalidRows = new ArrayList();
        for (int i = 1; i < table.getCells().size(); i++) {
            Expression expression = null;
            DataRow row = ((DataRow) table.getRow(i));
            expression = ConditionalStyleCompiler.getInstance().compile(row.getCellAt(0), s, uiController);
