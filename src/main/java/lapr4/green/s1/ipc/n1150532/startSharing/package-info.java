/**
 * Technical documentation regarding the user story <b>IPC01.1: Start
 * Sharing</b>.
 *
 * <h1>Requirements</h1>
 *
 ...
 * <h1>Analysis</h1>
 *
 * This use case requires a generic communication platform for the application,
 * which will be used by other use cases, as well as a cell sharing feature,
 * within the local network.
 *
 * <h2>Communication</h2>
 *
 * For the application to communicate, it is needed to broadcast a request for
 * other applications to provide their connection details, i.e. the port number
 * in which to send the connection request. This implies that the application
 * will also need a server to respond to such requests. The client must then
 * receive all the replies from the different server. Since it does not know how
 ...
 * Afterwards, it may select a range of {@link csheets.core.Cell}s whose
 ...
 * <h2>Special Notes</h2>
 *
 * This user story is related to all the other IPC use cases, since it provides
 * the communication base for them.
 *
 * The user story <b>IPC06</b> is strongly related to this one since that
 * requires an encryption technique. More information can be found there.
 *
 * <h1>Tests</h1>
 *
 ...
 * <h2>Servers</h2>
 *
 * Both UDP and TCP servers should be unique. An instance only needs one server
 * of a kind at any given time. In order to ensure this, the Singleton Pattern
 * can be used.
 *
 * <h2>Port Number</h2>
 *
 * The client requests the port number to be configurable. This seems like a
 * variation point among different clients and along the software's life time.
 * For these reasons, the Protected Variation Pattern can be applied. Although
 * it could be implemented with the Strategy Pattern, since it is not suppose to
 * change during runtime, it seems a little to excessive. A suitable solution is
 * to define the port number at the properties of the application. This provides
 * the versatility needed to change the value when needed at a low effort cost.
 * The port numbers for the UDP server and the TCP server must be different.
 *
 * <h2>Handlers</h2>
 *
 * The handlers are intended to deal with any DTO they might receive. The
 * workers must know which handler to use on each DTO. Since the right handler
 * to use is only known at runtime and it changes accordingly to the DTO
 * received, a suitable solution seems to be the Strategy Pattern. This pattern
 * can be implemented by using a map of handlers, whose key would be the DTO
 * class and the value the matching handler. In case there is no matching
 * handler, the received DTO will be ignored, i.e. no action will be performed.
 *
 * <h2>Find Peers</h2>
 *
 * The UDP client shall send a broadcast with a Connection Details Request DTO.
 * All the other instances UDP servers will respond with their IP address and
 * port number in which to send TCP connections within a Connection Details
 * Response DTO. The UDP client will process the replies for a brief defined
 * time, since it is impossible to know how many replies there will be. Each
 ...
 *
 * This instance will use a client worker to send a Connection Request DTO to
 * the other instance. The server of that instance will respond with a
 ...
 *
 * The connections made are to be maintained in order to use it for multiple
 * purposes. It will require the extension to keep track of their clients since
 * they are the bridge to the other instances. This implies a map, whose key is
 * the peer IP address and the value its dedicated worker. It must also allow
 * access to the worker output in order to send new DTO at any given time. This
 * implies a output stream getter within the worker.
 *
 * <h2>Terminate TCP Connections</h2>
 *
 * The use case does not explicitly request the connection to be closed.
 * However, it seems to be useful to find a way to do it, if the necessity
 * arises. The client will send a Close Connection Request DTO to the server.
 * The server will then check if its instance also has a client running. If so,
 * its client will also send a Close Connection Request DTO. Afterwards, the
 * server responds with a Close Connection Response DTO to the client and closes
 * the connection. The client receives the answer and closes its own connection.
 *
 * <h2>Share Cells</h2>
 *
 * For the time being only the value of the cell is to be transmitted, as well
 * as its address in order to place it in the right location. The client sends a
 * Cell Sharing DTO which the server will use to insert the values at the right
 * addresses. It will use the established communication structure.
 *
 * <h2>Encapsulation</h2>
 *
 * The handler may need some information from the server itself, specially to
 * get the addresses in order to perform the connections. A solution for it, is
 * to encapsulate the received DTO within another DTO built by the server which
 * contains the received DTO, the DTO with the server information to send and
 * the handler.
 *
 * <h2>Alternatives</h2>
 *
 * This design uses a dual connection between two peers. However, since the
 * server side can send requests to the client, it would be possible to work
 * with only one connection. For that, treating server and client alike would
 * spare that extra connection. This solution might be studied more deeply and
